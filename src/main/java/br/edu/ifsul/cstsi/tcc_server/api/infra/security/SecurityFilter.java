...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(tokenJWT != null){ //se a rota tem o token (no caso, qualquer rota, exceto /login)
            ...
            var user = repository.findByEmail(subject); //recupera o objeto usuário
            var authentication = new UsernamePasswordAuthenticationToken(user, null, user.getAuthorities()); //DTO do Spring para o usuário que acessa o sistema
